...
# Calculate the number of rows to select for the 10% subset
num_rows_subset <- ceiling(num_rows * 0.2)

# Randomly select the starting row index for the subset
start_row <- sample(1:(num_rows - num_rows_subset + 1), 1)

# Create the subset by selecting consecutive rows
subset_20_percent <- df_10 %>%
  slice(start_row:(start_row + num_rows_subset - 1))
...
# Calculate the number of rows to select for the 10% subset
num_rows_subset <- ceiling(num_rows * 0.3)

# Randomly select the starting row index for the subset
start_row <- sample(1:(num_rows - num_rows_subset + 1), 1)

# Create the subset by selecting consecutive rows
subset_30_percent <- df_10 %>%
  slice(start_row:(start_row + num_rows_subset - 1))
...
rest_predictions <- predict(model, newdata = df_10)


rmse <- rmse(rest_predictions, df_10$CHM)
r_squared <- cor(rest_predictions, df_10$CHM)^2
mae <- mae(rest_predictions, df_10$CHM)
